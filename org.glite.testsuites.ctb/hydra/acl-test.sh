...
RET1=grep "$ACL_TWO:--rw-x--" $TEMP_FILE
RET2=grep "$ACL_THREE:pd--l---" $TEMP_FILE
...
RET1=grep "$ACL_TWO:pdrwlxgs" $TEMP_FILE
RET2=grep "$ACL_THREE:pd--l---" $TEMP_FILE
...
set_acl -x "$ACL_TWO":gs testkey-1

get_acl testkey-1 $TEMP_FILE

RET1=grep "$ACL_TWO:------gs" $TEMP_FILE
RET2=grep "$ACL_THREE:pd--l---" $TEMP_FILE
...
if [ "$USE_TWO" -eq 1 ] ; then  
  ...
  glite-eds-key-unregister testkey-1
