...
int main (int argc, char** argv)
{
    ...
    if ( argc != 3 )
    {
        printf ("Usage: DPNS_modreplica <DPNS_HOST> <BASE_DIR>\n");
        return -1;
    }
    ...
    reportHeader ("* Executing dpns_modreplica tests...");

    // Test 1: Create a file, add replica and modify it
    strcpy (testdesc, "Test 1:Modify replica properties");
    sprintf (filename, "%s/dpns_modreplica", base_dir);
    ...
    ret = dpns_modreplica (sfn1, "setname_new_1", "poolname_new_1", "dpns_host_new_1");
    if ( ret != 0 )
    {
        error = 1;
    ...
    ret = dpns_modreplica (sfn2, "setname_new_2", "poolname_new_2", "dpns_host_new_2");
    if ( ret != 0 )
    {
        error = 1;
    ...
    ret = dpns_modreplica ("some_nonexisting_sfn", "setname_new_1", "poolname_new_1", "dpns_host_new_1");
    if ( ret != 0 )
    {
        if ( serrno == ENOENT )
        {
            reportComponent (testdesc, "dpns_modreplica returns -1, serrno is ENOENT", sstrerror(serrno), 0);
            goto test2_end;
        }
        else
        {
    ...
    // Test 3: Call dpns_modreplica with sfn == NULL 
    strcpy (testdesc, "Test 3:Call dpns_modreplica with sfn == NULL (EFAULT)");
    sprintf (filename, "%s/dpns_modreplica", base_dir);
    ret = dpns_creatx (filename, 0664, &dpns_fid);
    if ( ret != 0 )
    {
    ...
    ret = dpns_modreplica (NULL, "setname_new_1", "poolname_new_1", "dpns_host_new_1");
    if ( ret != 0 )
    {
        if ( serrno == EFAULT )
        {
            reportComponent (testdesc, "dpns_modreplica returns -1, serrno is EFAULT", sstrerror(serrno), 0);
            goto test3_end;
        }
        else
        {
    ...
    // Test 4: dpns_modreplica for a file when permission is not granted
    strcpy (testdesc, "Test 4:dpns_modreplica for a file when permission is not granted (EACCES)");
    sprintf (dirname, "%s/dir_modreplica", base_dir);
    ...
    sprintf (filename, "%s/dir_modreplica/dpns_modreplica", base_dir);
    ret = dpns_creatx (filename, 0664, &dpns_fid);
    if ( ret != 0 )
    {
        error = 1;
        reportComponent (testdesc, "Cannot create file dir_modreplica/dpns_modreplica", sstrerror(serrno), 1);
        goto test4_end;
    }
    ...
    if ( ret != 0 )
    {
        if ( serrno == EACCES )
        {
            reportComponent (testdesc, "dpns_modreplica returns -1, serrno is EACCES", sstrerror(serrno), 0);
            goto test4_end;
        }
        else
        {
    ...
    //Test 5: dpns_modreplica for sfn exceeding CA_MAXSFNLEN
    strcpy (testdesc, "Test 5:dpns_modreplica for sfn exceeding CA_MAXSFNLEN(ENAMETOOLONG)");
    sprintf (filename, "%s/modreplica_maxdfn", base_dir);
    ret = dpns_creatx (filename, 0664, &dpns_fid);
    if ( ret != 0 )
    {
        reportComponent (testdesc, "Cannot create file modreplica_maxsfn", sstrerror (serrno), 1);
        error = 1;
        goto test5_end;
    }
    ...
    ret = dpns_modreplica (sfnbig, "setname_new_1", "poolname_new_1", "dpns_host_new_1");
    if ( ret != 0 )
    {
        if ( serrno == ENAMETOOLONG )
        {
            reportComponent (testdesc, "dpns_modreplica returns -1, serrno is ENAMETOOLONG", sstrerror(serrno), 0);
            goto test5_end;
        }
        else
        {
    ...
    strcpy (testdesc, "Test 6:dpns_modreplica with setname exceeding 36(EINVAL)");
    sprintf (filename, "%s/modreplica_maxdfn", base_dir);
    ret = dpns_creatx (filename, 0664, &dpns_fid);
    if ( ret != 0 )
    {
    ...
    ret = dpns_modreplica ("some_file", setname, "poolname_new_1", "dpns_host_new_1");
    if ( ret != 0 )
    {
        if ( serrno == EINVAL )
        {
    ...
    //Test 7: dpns_modreplica with sername exceeding 36
    strcpy (testdesc, "Test 7:dpns_modreplica with poolname exceeding CA_MAXPOOLNAMELEN(EINVAL)");
    sprintf (filename, "%s/modreplica_maxdfn", base_dir);
    ret = dpns_creatx (filename, 0664, &dpns_fid);
    if ( ret != 0 )
    {
    ...
    ret = dpns_modreplica ("some_file", "setname_new_1", poolname, "dpns_host_new_1");
    if ( ret != 0 )
    {
        if ( serrno == EINVAL )
        {
    ...
    //Test 8: dpns_modreplica with sername exceeding 36
    strcpy (testdesc, "Test 8:dpns_modreplica with poolname exceeding CA_MAXPOOLNAMELEN(EINVAL)");
    sprintf (filename, "%s/modreplica_maxdfn", base_dir);
    ret = dpns_creatx (filename, 0664, &dpns_fid);
    if ( ret != 0 )
    {
    ...
    ret = dpns_modreplica ("some_file", "setname_new_1", "poolname_new_1", hostname);
    if ( ret != 0 )
    {
        if ( serrno == EINVAL )
        {
    ...
    strcpy (testdesc, "Test 9:Call dpns_modreplica with unknown DPNS_HOST(SENOSHOST)");
    setenv ("DPNS_HOST", "host.which.does.not.exist", 1);
    ret = dpns_modreplica ("a", "b", "c", "d");
    if ( ret != 0 )
    {
        if ( serrno == SENOSHOST )
