...
blex_holdrelease_test_n_sleep=300
...
# One percent tolerance
  lc_allowed_diff=$(( $blex_holdrelease_test_n_sleep/100 + 1))
  lc_delta=$(( lc_end - lc_start ))
  lc_diff=$(( lc_delta - blex_holdrelease_test_n_sleep ))
  if [ $lc_diff -lt 0 ]
  then
    let lc_diff=-lc_diff
  fi
  if [ $lc_diff -gt $lc_allowed_diff ]
  then
    lc_fcheck="${lc_fcheck}ERROR: Time difference ($lc_delta s) is too large (expected $blex_holdrelease_test_n_sleep +/- $lc_allowed_diff "
  fi
