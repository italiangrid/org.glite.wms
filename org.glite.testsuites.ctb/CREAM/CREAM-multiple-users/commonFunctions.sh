...
do
  ...
  do
    ...
    echo $TESTUSERMAGIC | voms-proxy-init -valid 24:00 --voms $vo -key test_user_${index}_key.pem -cert test_user_${index}_cert.pem  -out proxy_${index} -pwstdin 1>>/dev/null 2>>/dev/null
    if [ $? -ne 0 ]; then
...
do
  ...
  do
    ...
    do
      echo "Creating proxy for test user ${index}"
      echo $TESTUSERMAGIC | voms-proxy-init -valid 24:00 --voms $vo:/$vo/Role=$role -key test_user_${index}_key.pem -cert test_user_${index}_cert.pem  -out proxy_${index} -pwstdin 1>>/dev/null 2>>/dev/null
      if [ $? -ne 0 ]; then
        echo "Error creating proxy"
        exit 1
      fi
