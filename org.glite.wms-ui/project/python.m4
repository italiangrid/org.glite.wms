...
[
    ...
    AC_PATH_PROG(PPYTHON,python2.2,no,$PYTHON_BIN_PATH)
	...
	changequote(<<, >>)
        ...
        if test $python_major_ver -eq $ac_req_major -a $python_minor_ver -eq $ac_req_minor ; then
            result=yes
        else 
            if test $python_major_ver -eq $ac_req_major -a $python_minor_ver -eq $ac_req_minor -a $python_micro_ver -eq $ac_req_micro ; then 
                result=yes
            fi
        fi
    else
    ...
    if test -n "$with_python_prefix" -a "x$result" = "xyes" ; then
        PYTHON_CFLAGS=-I$with_python_prefix/include/python$ac_req_major.$ac_req_minor
    else
        PYTHON_CFLAGS=""
    fi
