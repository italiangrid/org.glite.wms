...
namespace socket_pp = tls::socket_pp;
...
public:
   ...
   */
  NSClient(const std::string& h, int p, logger::level_t lvl = logger::null);
   ...
   * List Job Match Command.
   * @param jdl the JDL of the job being submitted.
   * @param l a pointer to a vector of couples which will receive CEIds matching the jdl and rank.
   * @return whether the request has been correctly sent to the remote NS.
   */
