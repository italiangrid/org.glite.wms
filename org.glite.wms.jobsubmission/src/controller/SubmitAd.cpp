...
void SubmitAd::createFromAd(classad::ClassAd* pad)
{
  ...
  if (this->sa_good) {
    ...
    if (!this->sa_hasDagId) {
      ...
      dagId.fromString(this->sa_dagid);
    }

    this->sa_jobid = glite::jdl::get_edg_jobid(*this->sa_ad, this->sa_good);
    if (!this->sa_good) {
      this->sa_reason = "Cannot extract \"edg_jobid\" from given classad.";
    } else {
...
//    this->sa_classadfile.assign( files->classad_file().native_file_string());
    ...
    if (this->sa_jobtype == "dag") {
        ...
        if (!this->sa_hasDagId) { // Create CondorG log file name
      ...
      this->sa_logfile = files->log_file(epoch).native_file_string();
    }
...
SubmitAd::SubmitAd(classad::ClassAd* pad)
  : sa_good(true), sa_last(false), sa_hasDagId(false), sa_isDag(false),
    sa_jobperlog(1), sa_lastEpoch(0), sa_ad(pad),
    sa_jobid(), sa_dagid(), sa_jobtype(),
    sa_submitfile(), sa_submitad(), sa_reason(),
    sa_seqcode(), sa_classadfile(), sa_logfile()
{ }

SubmitAd &SubmitAd::set_sequence_code(string const& code)
{
  string     seqcode, notes;

  this->sa_seqcode = code;
  if (this->sa_good) {
    if (0 == this->sa_seqcode.size()) {
      seqcode = glite::jdl::get_lb_sequence_code(*this->sa_ad);
    } else {
      seqcode = this->sa_seqcode;
    }

    if(this->sa_jobtype == "job") {
      notes = "(" + this->sa_jobid + ") (" + seqcode
        + ") (" + boost::lexical_cast<string>(this->sa_last) + ')';
    ...
    } else if ("dag" == this->sa_jobtype) {
      notes = "DAG job: (" + this->sa_jobid + ") (" + seqcode + ')';
      glite::jdl::set_lb_sequence_code( *this->sa_ad, seqcode );
      glite::jdl::set_submit_event_notes( *this->sa_ad, notes );
    }
  }
