...
import org.glite.security.voms.VOMSAttribute ; //  getVO()
import org.glite.security.util.DNHandler ;  // DN X509 expression handler
import org.glite.security.voms.VOMSValidator ;  // Voms info parser
import org.glite.security.voms.FQAN ; //Voms group parser
...
public class UserCredential{
	...
	*/
	...
	throws FileNotFoundException , IOException, java.security.GeneralSecurityException, GlobusCredentialException{
		...
		org.globus.gsi.bc.BouncyCastleOpenSSLKey key = new org.globus.gsi.bc.BouncyCastleOpenSSLKey(userKey);
		CertUtil.init(); // Loads and initializes class.
		if (key.isEncrypted()) {
			key.decrypt(passPhrase);
		}
		org.globus.gsi.bc.BouncyCastleCertProcessingFactory factory = org.globus.gsi.bc.BouncyCastleCertProcessingFactory.getDefault();
		GlobusCredential proxy = factory.createCredential(new X509Certificate[] {CertUtil.loadCertificate(userCert)},
	...
	* @return the string representation of the proxy file name
	*/
	public String getUserProxy () throws FileNotFoundException{
		if (userProxy==null){
	...
	*/
	public String getX500UserSubject() throws GlobusCredentialException {
		return DNHandler.getDN( proxy.getSubject()).getX500()  ;
	}
	/**
	...
	*/
	public static String getX500UserSubject(String proxySubject) {
		return DNHandler.getDN(proxySubject).getX500()  ;
	}
	/****************************************
	...
	*/
	public Vector getVONames() throws FileNotFoundException, GlobusCredentialException, Exception {
		java.util.List infoList =getProxyVOMSValidator().getVOMSAttributes() ;
		Vector voVector = new Vector() ;
		for (int i = 0; i < infoList.size(); i++) {
			voVector.add(((VOMSAttribute) infoList.get(i)).getVO());
		}
		return voVector;
	}
	/**
	...
	*/
	public String getDefaultVOName() throws IOException, GlobusCredentialException, Exception {
		...
		else return   ((VOMSAttribute)(infoList.get(0))   ).getVO();
	}
	/**
	...
	*/
	public Vector getGroups(String voName) throws IOException, GlobusCredentialException, Exception {
		java.util.List infoList =getProxyVOMSValidator().getVOMSAttributes() ;
		VOMSAttribute vomsInfo;
		Vector groupVector = new Vector();
		for (int i = 0; i < infoList.size(); i++) {
			if (((VOMSAttribute) infoList.get(i)).getVO().trim().equals(voName.trim())) {
				java.util.List groupsList = ((VOMSAttribute) infoList.get(i)).getListOfFQAN() ;
				for (int j = 0; j < groupsList.size(); j++) {
					groupVector.add(   ((FQAN)groupsList.get(j)).getGroup()   );
				}
				break;
			}
		}
	...
	* @throws Exception
	* @param voName the name of the Virtual Organisation.
	* @return true, if the proxy is a VOMS proxy and voName is present, false otherwise.
	*/
	...
	* @throws GlobusCredentialException - Unable to load proxy certificate.
	* @throws Exception
	* @return true, if the proxy has a VOMS extension, false otherwise.
	*/
	public boolean hasVOMSExtension() throws IOException, GlobusCredentialException, Exception {
		return (  !getProxyVOMSValidator().getVOMSAttributes().isEmpty() ) ;
	}
	private VOMSValidator getProxyVOMSValidator() throws FileNotFoundException, GlobusCredentialException, Exception {
			String proxyName = "";
			try {
				if ((this.userProxy != null) && !this.proxy.equals(""))  proxyName = this.userProxy;
				else  proxyName = getDefaultProxyName();
				return new VOMSValidator ( proxy.getCertificateChain()  ).parse();
