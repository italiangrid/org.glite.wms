  ...
  time_t timeout, ism::purchaser::PurchaserInfoContainer& se_info_container) 
{
  static char const* filter(
    "(|(objectclass=gluecesebind)(|(objectclass=gluese)"
  ...
  result = ldap_search_st(
    ld.get(), dn.c_str(), LDAP_SCOPE_SUBTREE, filter, 0, false, &to, &ldresult
  );

  if (result != LDAP_SUCCESS ) {
     Error("ldap_search_st: " << ldap_err2string(result));
    return;
  }
...
void 
fetch_bdii_ce_info(
  std::string const& host, size_t port, std::string const& dn,time_t timeout,
