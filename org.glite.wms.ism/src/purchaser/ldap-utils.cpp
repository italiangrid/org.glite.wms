...
namespace classadutils = wmsutils::classads;
namespace wms {
...
typedef std::map<
  std::string, // ce id
  std::vector<pair_string_ad> // list of ce views per vo
  ...
  glue_info_container_type& gluece_info_container) 
{
  ...
  for ( ; cl_it != cl_e; ++cl_it) { // for each cluster
    ...
    if (boost::tuples::get<2>(cl_it->second).empty()) {
      ...
      Info("Skipping cluster " << cl_it->first
    ...
    for ( ; ce_it != ce_e; ++ce_it) {
        ...
        !vo_views->second.empty())
      {
        ...
        vector<pair_string_ad>::const_iterator
          vo_view_it(vo_views->second.begin());
        vector<pair_string_ad>::const_iterator const
        ...
        for ( ; vo_view_it!=vo_view_e; ++vo_view_it) {
          ...
          {
            Info("Skipping view " << vo_view_it->first
