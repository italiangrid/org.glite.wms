...
//     Error("ldap_simple_bind_s failure: " << ldap_err2string(result));
  ...
  SEInfoMap  se_info;
  std::time_t const absolute_timeout = std::time(0) + timeout; // seconds
  boost::timer t0;
  size_t n_entries = 0;
  while ( (result = ldap_result(
    ...
    if (result != LDAP_RES_SEARCH_ENTRY) continue; // timeout or actual an entry actually
  ...
  Debug("#" << n_entries << " LDAP entries received in " << t0.elapsed() << " seconds");
  ut::scope_guard ldresult_guard(
    boost::bind(ldap_msgfree, ldresult)
  );
  if (result == -1) {
...
*/
  ...
  Debug("ClassAd reppresentation built in " << t1.elapsed() << " seconds");
}
  ...
  time_t timeout, ism::purchaser::PurchaserInfoContainer& ce_info_container) 
{
  ...
  if (filter_ext) { filter += "(&(|"; };
  filter += "(objectclass=gluevoview)(objectclass=gluece)";
  if (filter_ext) { filter.append(")").append(filter_ext).append(")"); }
  filter += ")";
