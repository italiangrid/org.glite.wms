...
void parse_ldif_event_messages(CEEvent* ceE, gluece_info_container_type& gluece_info_container)
{
  ceE->getEvent();
  size_t n = 0;
...
bool ism_cemon_purchaser_entry_update::operator()(int a,boost::shared_ptr<classad::ClassAd>& ad)
{
...
void ism_cemon_purchaser::do_purchase()
{
  ...
  do {
	...
	Error("Unable to get event from CEMonitor:" << e.getMethod() << ", " << e.getErrorCode() << ", " <<
	...
	if (expand_glueceid_info(it->second)) {
          ...
          get_ism()[it->first] = boost::make_tuple(static_cast<int>(get_current_time().sec), TTLCEinfo, it->second, ism_cemon_purchaser_entry_update());
        }
      }
    }
