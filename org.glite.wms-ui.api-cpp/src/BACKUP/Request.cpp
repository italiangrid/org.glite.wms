#include "edg/workload/userinterface/client/Request.h"
...
#include "edg/workload/common/requestad/jdl_attributes.h"
...
/********************** VARIOUS *************************************/
#include "edg/workload/common/jobid/manipulation.h"  // to_filename method
...
void Request::getOutput(const std::string& dir_path){
	...
	EDG_STACK_TRY("Request::getOutput(const std::string& nsHost , int nsPort , const std::string& lbHost , int lbPort)");
	...
	nsClient =   new glite::wms::manager::ns::client::NSClient   ( nsHost, nsPort , (edg::workload::common::logger::level_t) loggerLevel)    ;
...
void Request::getOutput(const std::string& dir_path , const std::string& nsRootPath  , const  glite::wms::lb::JobStatus& status ){
	using namespace glite::wms::lb ;
	EDG_STACK_TRY("Request::getOutput(const std::string& dir_path , const std::string& nsRootPath  , const JobStatus& status ) ");
...
JobId Request::submit(const std::string& nsHost , int nsPort , const std::string& lbHost , int lbPort , const std::string& ceid){
	EDG_STACK_TRY("Request::submit(const std::string& nsHost , int nsPort , const std::string& lbHost , int lbPort)");
...
std::vector<std::string> Request::listMatchingCE(const std::string& nsHost , int nsPort ){
	EDG_STACK_TRY("Request::listMatchingCE(const string& host , int port)") ;
...
glite::wms::lb::JobStatus Request::getStatus(bool ad)  {
	EDG_STACK_TRY("Request::getStatus(bool ad)")  ;
	using namespace glite::wms::lb ;
	switch (type){
		case EWU_TYPE_NONE:
...
std::vector <glite::wms::lb::Event> Request::getLogInfo() {
	EDG_STACK_TRY("Request::getLogInfo()")   ;
	using namespace glite::wms::lb ;
	switch (type){
		case EWU_TYPE_NONE:
		case EWU_TYPE_DAG_AD:
