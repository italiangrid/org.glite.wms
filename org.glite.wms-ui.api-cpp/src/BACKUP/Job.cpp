...
Job::Job(const JobId& id){
     EDG_STACK_TRY("Job::Job(const JobId& id)");
     if (!  ((JobId) id).isSet() )
         throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Empty JobId instance" ) ;
     else {
...
Job::Job(const JobAd& ad){
      EDG_STACK_TRY("Job::Job(const JobAd& ad)") ;
      if (!  ((JobAd) ad).isSet() )
          throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Empty JobAd instance" ) ;
      else{
...
Job::Job(const Job& job) {
   EDG_STACK_TRY("Job::Job(const Job& )") ;
   jid = NULL ;
   jad = NULL ;
   jobType = job.jobType ;
...
void Job::operator=(const Job& job) {
    EDG_STACK_TRY("Job::operator=(const Job& job)") ;
    if (jid) delete jid  ;
    if (jad) delete jad ;
    jid = NULL ;
    jad = NULL ;
...
void Job::setJobAd(const JobAd& ad)  {
   EDG_STACK_TRY("Job::setJobAd(const JobAd& ad)") ;
   if  (!jid->isSet()){
      if (jad) delete jad ;
      this->jad =  new JobAd (ad) ;
      this->jad->check();
     ...
     throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED, "JobId instance already set" ) ;
 EDG_STACK_CATCH() ; //Exiting from method: remove line from stack trace
}
void Job::setJobId(const JobId& id)  {
   EDG_STACK_TRY("Job::setJobId(const JobId& id)") ;
   ...
   if  (jad  == NULL){
     ...
     throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED, "JobAd instance already set" ) ;
   EDG_STACK_CATCH() ; //Exiting from method: remove line from stack trace
}


void Job::retrieveJobAd() {
   EDG_STACK_TRY("Job::retrieveJobAd()")
   //Check if the JobId has been already set
   if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
              // Operation not allowed
              throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "retrieveJobAd not allowed" ) ;
...
JobState Job::getState( unsigned int step )  {
  EDG_STACK_TRY("Job::getState(unsigned int step)")  ;
  if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
        // JobId not set yet
        throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "getState not allowed" ) ;
  ...
  if (  !jad->hasAttribute(JDL::JOBTYPE , JDL_JOBTYPE_CHECKPOINTABLE )  )
     throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Checkpointable retrieval not allowed: not a chkpt job" ) ;
  ...
  if ( error == ENOENT )  // no events found
   // return "";
   throw JobOperationException( __FILE__, __LINE__, METHOD ,WL_JOBOP_ALLOWED, "No events found: ENOENT");
  if ( error )  // query failed
    throw JobOperationException( __FILE__, __LINE__, METHOD,WL_JOBOP_ALLOWED ,"Query failed");
  for ( cnt=0; events[cnt].type; cnt++ ); // counts the number of events
  if ( !cnt ) // no events found
    throw JobOperationException( __FILE__, __LINE__, METHOD , WL_JOBOP_ALLOWED, "Empty Events vector returned");
  ...
  if ((int)step>= cnt)
    throw JobOperationException( __FILE__, __LINE__, METHOD , WL_JOBOP_ALLOWED, "Number of step bigger then chkpt logged events");
...
int Job::attach ( Listener* ls , int port) {
  EDG_STACK_TRY("Job::attach(int port)")  ;
  //  body TBD:
  if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
        // JobId not set yet
        throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Attach not allowed" ) ;
      ...
      switch (   stat.status  ){
		...
		if (stat.getValInt ( JobStatus::DONE_CODE) ==1)
         ...
         default:  // Any other value: CLEARED ,ABORTED ,CANCELLED, PURGED
              // Operation not allowed
              throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Attachement not allowed: check the status" ) ;
      }
      // Initialising internal JobAd:
      jad->fromString (stat.getValString(JobStatus::JDL ) );
  }
  if (  !jad->hasAttribute(JDL::JOBTYPE , JDL_JOBTYPE_INTERACTIVE )  )
     throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Attachment not allowed: not an interactive job" ) ;
  sh.set(  *jid , ls) ;
  sh.console(  port );
  // cout << "Check wheter the nsHost has been initialised" << endl << flush ;
  lbInit( nsHost) ;
  if   (   jobType != JOB_SUBMITTED){
          // cout << "\n                    LB-DBG: "<< "edg_wll_SetLoggingJob"  << jid->toString() << flush ;
          if ( edg_wll_SetLoggingJob(ctx , jid->getId() , NULL , EDG_WLL_SEQ_DUPLICATE) )
               throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "LB  edg_wll_SetLoggingJob  failed" ) ;
  }
  // cout << "\n                    LB-DBG: "<< "edg_wll_LogListener" << flush ;
  if (edg_wll_LogListener(  ctx , "InteractiveListener", sh.getHost().c_str(), (uint16_t) sh.getPort()) )
          throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "LB edg_wll_LogListener  failed" ) ;
  sh.start ( ) ;
  return 0 ;
  EDG_STACK_CATCH() ; //Exiting from method: remove line from stack trace
}
...
JobStatus Job::getStatus(bool ad)  {
   EDG_STACK_TRY("Job::getStatus(bool ad)")  ;
   // if (jCollect) cout << "\n  Job::getStatus   " << endl << flush ;
   if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
        // Operation not allowed
        throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "getStatus not allowed" ) ;
   if (! jCollect)
...
vector<Event> Job::getLogInfo() {
   EDG_STACK_TRY("Job::getLogInfo()")   ;
     if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
        // Operation not allowed
        throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "getLogInfo not allowed" ) ;
     userCred.checkProxy(cred_path) ;
...
void Job::submit ( const string& host, int port,  const string& lbHost , int lbPort , JobState *state , Listener *listener , const string& ce_id)  {
     EDG_STACK_TRY("Job::submit(const string& , const string& )")
     if   (   jobType != JOB_AD  )
...
vector<pair < string, double> > Job::listMatchingCE(const string& host , int port  ) {
	EDG_STACK_TRY("Job::listMatchingCE(const string& host , int port )")
	vector<pair <string , double> >resources;
	if   (   jobType == JOB_NONE )
		//  Liust Match is not allowed
		throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "List Match not allowed" ) ;
	nsInit( host , port );
	nsList(resources);
	return resources;
	EDG_STACK_CATCH() ; //Exiting from method: remove line from stack trace
...
ResultCode Job::cancel() {
  EDG_STACK_TRY("Job::cancel()")
  if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
         // Operation not allowed
         throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Cancel not allowed" ) ;
  // Check if the status of the Job allows to cancel it
  JobStatus stat = getStatus( );
  switch (   stat.status  ){
         case JobStatus::SUBMITTED:
		...
		if (stat.getValInt ( JobStatus::DONE_CODE) ==1)
         ...
         default:  // Any other value: CLEARED ,ABORTED ,CANCELLED, PURGED
              // Operation not allowed
              throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Cancel not allowed: check the status" ) ;
  }               
  if (  stat.getValBool (JobStatus::CANCELLING )  )
     throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Cancel has been already requested" ) ;
     // return ACCEPTED ;
...
void Job::getOutput(const string& dir_path) {
   EDG_STACK_TRY("Job::getOutput(const string& dir_path)")
   if   (   jobType == JOB_NONE ||  jobType == JOB_AD  )
      // Operation not allowed
      throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Output not allowed" ) ;
   ...
   if (jobType != JOB_SUBMITTED){
	...
	if (!jad-> hasAttribute (JDL::OUTPUTSB)){
		throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "The Job has no output files to be retrieved" ) ;
	}
...
glite::wmsutils::jobid::JobId* Job::submit(
	...
	int timeout ,
	int time_interval){
	EDG_STACK_TRY("static JobId* Job::submit " ) ;
	JobAd ad ;
	...
	if ( cycles == timeout )
		throw JobOperationException ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "Timeout exceeded for output retrieval" ) ;
...
void Job::nsInit(const string& host ,int  port){
	EDG_STACK_TRY("Job::nsInit(const string& nsAddress,int nsPort)")
	if (! jCollect)  userCred.checkProxy(cred_path) ;
	nsHost = host ;
	nsPort = port ;
	nsClient = new NSClient   (nsHost, nsPort , (edg::workload::common::logger::level_t)loggerLevel)    ;
	EDG_STACK_CATCH() ; //Exiting from method: remove line from stack trace
};
void Job::nsList( vector<pair <string, double> > &resources ) {
	EDG_STACK_TRY("Job::nsList(vector<string> *resources )")
	vector<string> multi ;
...
void Job::nsSubmit(const string&  lb_addr ) {
	...
	try{
		...
		throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , exc.what()  ) ;
	}
	// OutputData changes:
	...
	if (!jad->hasAttribute ( JDL::VIRTUAL_ORGANISATION)  )
     ...
     if (edg_wll_RegisterJobSync ( ctx , jid->getId() ,   EDG_WLL_JOB_SIMPLE ,  jdl.c_str(),  str_addr, 0 ,   NULL, NULL) ){
           ...
           throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED ,  error_message) ;
     }
     ...
     if (  jad->hasAttribute( JDL::JOBTYPE , JDL_JOBTYPE_CHECKPOINTABLE)  ){
        ...
        if (edg_wll_LogEventSync( ctx, EDG_WLL_EVENT_CHKPT ,EDG_WLL_FORMAT_CHKPT , current_step.c_str() , jad->getAd(JDL_CHKPT_JOBSTATE).c_str() ) )
              throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "LB  edg_wll_LogEventSync failed" ) ;
	...
	if (  jad->hasAttribute(JDL::USERTAGS )  ){
		...
		for (unsigned int i = 0 ; i< vect.size() ; i++){
			if ( !userTags->EvaluateExpr( vect[i].second, val ) )
				...
				throw JobOperationException     ( __FILE__ , __LINE__ ,  "Logging::logUserTags" , WL_JOBOP_ALLOWED , "Unable to Parse Expression" ) ;
			...
			if  ( val.IsStringValue (attrValue)  )
				if (      edg_wll_LogUserTag( ctx,  (vect[i].first).c_str() ,  attrValue.c_str()  )     )
					throw JobOperationException     ( __FILE__ , __LINE__ ,  "Logging::logUserTags",WL_JOBOP_ALLOWED , "edg_wll_LogUserTag failed") ;
...
void Job::nsOutput( const string& finalDir ){
	EDG_STACK_TRY("Job::nsOutput( const string& jobId , const string& dir )")
...
void Job::lbInit(  const string& nsHost  ) {
     ...
     ) throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "LB initialisation failed" ) ;
     if   ( ! getenv ( EDG_WL_LOG_DESTINATION) )
          if (edg_wll_SetParamString( ctx, EDG_WLL_PARAM_DESTINATION, nsHost.c_str() ) )
               throw JobOperationException     ( __FILE__ , __LINE__ ,METHOD , WL_JOBOP_ALLOWED , "LB initialisation failed (set destination)" ) ;
}
