...
#include "edg/workload/userinterface/client/JobCollection.h"
...
// requestad
#include "edg/workload/common/requestad/JDLAttributes.h"
#include "edg/workload/common/jobid/JobId.h"
// NetworkServer
#include "edg/workload/networkserver/client/NSClient.h"
...
JobCollection::JobCollection() {
  // SSL thread Safe
  EDG_STACK_TRY("JobCollection::JobCollection()") ;
  Job::initialise() ;
  maxThreadNumber = 1 ;
...
JobCollection::JobCollection(const vector<Job>& jobs) {
  EDG_STACK_TRY("JobCollection::JobCollection(const vector<Job>& jobs)") ;
...
JobCollection::JobCollection(const Job& job , unsigned int n) {
 EDG_STACK_TRY("JobCollection::JobCollection(const Job& job , unsigned int n) ") ;
...
void JobCollection::insert(const Job& job){
	EDG_STACK_TRY("JobCollection::insert(const Job& job)") ;
...
void JobCollection::remove (const Job& job){
     EDG_STACK_TRY("JobCollection::remove (const Job& job)") ;
     vector<Job>::iterator   jobIt;
     bool cleared = false ;
     string jobString;
     try{
...
CollectionResult  JobCollection::getStatus(   ){
     EDG_STACK_TRY("JobCollection::getStatus(vector <pair< Job , JobStatus > >)") ;
...
CollectionResult JobCollection::submit(const string& ns_host , int ns_port , vector < pair < string , int > > lbAddrs , const string& ce_id ){
  EDG_STACK_TRY("JobCollection::submit(const string& ns_host, int ns_port , const string& lbHost , int lbPort , const string& ce_id )") ;
  operation = SUBMIT ;
...
CollectionResult  JobCollection::cancel( ){
   EDG_STACK_TRY("JobCollection::cancel( const string &email)") ;
   operation = CANCEL ;
   paramStruct       ps   ;
   return launch (ps) ;
   EDG_STACK_CATCH() ; //Exiting from method: remove line from stack trace
...
CollectionResult  JobCollection::getOutput(const string& dir_path){
 EDG_STACK_TRY("JobCollection::getOutput(const string& dir_path)") ;
   operation = OUTPUT ;
   paramStruct       ps   ;
...
pthread_t JobCollection::ExecuteThread(void* (*fn)(void*), void *arg) {
    EDG_STACK_TRY("JobCollection::ExecuteThread(void* (*fn)(void*), void *arg)");
...
resultStruct   JobCollection::retrieve ( pthread_t tid ,  int  jobNumber){
     EDG_STACK_TRY("JobCollection::retrieve ( pthread_t tid , int  jobNumber)") ;
...
CollectionResult JobCollection::launch( const paramStruct  &params ){
     EDG_STACK_TRY("JobCollection::launch( const paramStruct &params )") ;
...
void* JobCollection::submitTo  (void* paramStruct) {
	EDG_STACK_TRY("JobCollection::submitTo  (void* paramStruct)") ;
	...
	EDG_STACK_CATCH() ;  //remove last SET_NAME (exited with throw)
};
...
void* JobCollection::cancelTo (void* ps) {
	EDG_STACK_TRY("JobCollection::cancelTo (void* ps)") ;
	try{
...
void* JobCollection::statusTo (void* paramStruct) {
	EDG_STACK_TRY("JobCollection::statusTo (void* paramStruct)") ;
	try{
		glite::wms::lb::JobStatus *status  =   new   edg::workload::logging::client::JobStatus(  ) ;
...
void* JobCollection::getOutputTo (void* paramStruct) {
	EDG_STACK_TRY("JobCollection::getOutputTo (void* paramStruct)");
