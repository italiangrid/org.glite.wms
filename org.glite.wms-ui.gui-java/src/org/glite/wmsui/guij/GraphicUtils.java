...
public class GraphicUtils {
  ...
  static final KeyStroke CUT_ACCELERATOR =
      KeyStroke.getKeyStroke(KeyEvent.VK_X,
      KeyEvent.CTRL_MASK | KeyEvent.SHIFT_DOWN_MASK);
  static final KeyStroke COPY_ACCELERATOR =
      KeyStroke.getKeyStroke(KeyEvent.VK_C,
      KeyEvent.CTRL_MASK | KeyEvent.SHIFT_DOWN_MASK);
  static final KeyStroke PASTE_ACCELERATOR =
  ...
  static int showOptionDialogMsg(Component component, String inputMsg,
      ...
      String lastMessageLine) throws IllegalArgumentException {
    ...
    if (messageVectorSize > 0) {
      ...
      if (messageVectorSize == 1) {
        int choice = jOptionPane.showOptionDialog(component,
            firstMessageLine + messageVector.get(0).toString()
            + lastMessageLine,
            title,
            optionType,
            messageType,
        ...
        switch (optionType) {
          case JOptionPane.DEFAULT_OPTION: {
            ...
            while ((choice == 3) || (choice == 4)) {
              count = "Message Page: " + (index + 1) + "/" + messageVector.size();
              count = "<html><font color=\"#602080\">" + count + "</font>";
              jLabelCount.setText(count);
              choice = jOptionPane.showOptionDialog(component,
                  firstMessageLine
                  + messageVector.get(index).toString()
                  + lastMessageLine,
                  title,
                  optionType,
                  messageType,
  ...
  public static void windowCenterWindow(Window parentWindow, Window window) {
    ...
    int x = (parentWidth > windowWidth) ?
        (parentWidth - windowWidth) / 2 + parentX
        : windowX - (windowWidth - parentWidth) / 2;
    int y = (parentHeight > windowHeight) ?
        (parentHeight - windowHeight) / 2 + parentY
        : windowY - (windowHeight - parentHeight) / 2;
