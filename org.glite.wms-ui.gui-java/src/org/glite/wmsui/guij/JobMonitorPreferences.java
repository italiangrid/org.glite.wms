...
public class JobMonitorPreferences extends JDialog {
      ...
      try {
        ...
        if (userConfAd.hasAttribute(Utils.PREF_FILE_JOB_MONITOR)) {
          ...
          if (userConfJobMonitorAd.hasAttribute(Utils.PREF_FILE_UPDATE_RATE)) {
            updateRate = ((Integer) userConfJobMonitorAd.getIntValue(Utils.PREF_FILE_UPDATE_RATE).get(0)).intValue();
          }
        }
      }
      catch (Exception ex) {
      ...
      try {
        ...
        if (userConfAd.hasAttribute(Utils.PREF_FILE_JOB_MONITOR)) {
          ...
          if (userConfJobMonitorAd.hasAttribute(Utils.PREF_FILE_LB_ADDRESS)) {
            Vector lbAddressVector = userConfJobMonitorAd.getStringValue(Utils.PREF_FILE_LB_ADDRESS);
  ...
  static void savePrefFile(Vector lbVector, Integer updateRate) throws Exception {
    Ad userConfAd = new Ad();
    Ad userConfJobMonitorAd = new Ad();
    File userConfFile = new File(GUIFileSystem.getUserPrefFile());
    if(userConfFile.isFile()) {
    ...
    if (updateRate != null) {
      try {
        ...
        userConfJobMonitorAd.setAttribute(Utils.PREF_FILE_UPDATE_RATE, updateRate.intValue());
      } catch (Exception ex) {
        if (isDebugging) ex.printStackTrace();
      }
    }
    try {
      userConfAd.setAttribute(Utils.PREF_FILE_JOB_MONITOR, userConfJobMonitorAd);
    } catch (Exception ex) {
      if (isDebugging) ex.printStackTrace();
    }
    try {
  ...
  static void initializeUserConfiguration() {
    ...
    try {
      if (userPrefAd.hasAttribute(Utils.PREF_FILE_JOB_MONITOR)) {
        Ad monitorAd = (Ad) userPrefAd.getAdValue(Utils.PREF_FILE_JOB_MONITOR).get(0);
        logger.debug("initializeUserConfiguration() monitorAd: " + monitorAd);
        if (monitorAd.hasAttribute(Utils.PREF_FILE_UPDATE_RATE)) {
          GUIGlobalVars.setJobMonitorUpdateRate(((Integer) monitorAd.getIntValue(
              Utils.PREF_FILE_UPDATE_RATE).get(0)).intValue());
          logger.debug("initializeUserConfiguration pref file update rate: " + ((Integer) monitorAd.getIntValue(
              Utils.PREF_FILE_UPDATE_RATE).get(0)).intValue());
        }
      }
    } catch (Exception e) {
