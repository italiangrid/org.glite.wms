...
public class StatusDetailsFrame extends JFrame {
  ...
  static int[] primaryAttNames = {
      JobStatus.JOB_ID, JobStatus.STATE_ENTER_TIME, JobStatus.REASON,
      JobStatus.STATUS,
      JobStatus.JOBTYPE, JobStatus.EXIT_CODE, JobStatus.OWNER,
      JobStatus.NETWORK_SERVER, JobStatus.DESTINATION,
      JobStatus.CONDOR_JDL, JobStatus.JDL, JobStatus.MATCHED_JDL, JobStatus.RSL //,
      /* To remove even if not present in primary panel*/
      /*JobStatus.STATUS_CODE*/
  };

  static final int[] statusAttributeIndex = {
      JobStatus.ACL,
      JobStatus.CANCEL_REASON, JobStatus.CANCELLING, JobStatus.CE_NODE,
      JobStatus.CHILDREN,
      JobStatus.CHILDREN_HIST, JobStatus.CHILDREN_NUM,
      JobStatus.CHILDREN_STATES, JobStatus.CONDOR_ID,
      JobStatus.CONDOR_JDL, JobStatus.CPU_TIME, JobStatus.DESTINATION,
      JobStatus.DONE_CODE,
      JobStatus.EXIT_CODE, JobStatus.EXPECT_FROM, JobStatus.EXPECT_UPDATE,
      JobStatus.GLOBUS_ID,
      JobStatus.JDL, JobStatus.JOB_ID, JobStatus.JOBTYPE,
      JobStatus.LAST_UPDATE_TIME,
      JobStatus.LOCAL_ID, JobStatus.LOCATION, JobStatus.MATCHED_JDL,
      JobStatus.NETWORK_SERVER,
      JobStatus.OWNER, JobStatus.PARENT_JOB, JobStatus.REASON,
      JobStatus.RESUBMITTED,
      JobStatus.RSL, JobStatus.SEED, JobStatus.STATE_ENTER_TIME,
      JobStatus.STATE_ENTER_TIMES,
      JobStatus.SUBJOB_FAILED, JobStatus.USER_TAGS, /*JobStatus.USER_VALUES*/
      JobStatus.USER_TAGS
  };

  static final String[] statusAttributeDescr = {
      "Access Control List",
      "Cancellation Reason", "Cancellation in progress",
      "Execution Worker Node", "Subjobs Id list",
      "Subjobs States Summary", "Subjobs Number", "Subjobs States",
      "Condor-G Job Id",
      "Condor-G Job Description", "Consumed CPU Time",
      "Destination CE Identifier", "Done Code",
      "Job Exit Code", "Source of Missing Information",
      "Some info has not arrived yet", "Globus Job Id",
      "User Job Description", "Job Identifier", "Job Type",
      "Last Job Event Time",
      "LRMS Job Id", "Job Current Location",
      "Job Description after Matchmaking", "Network Server",
      "Job Owner", "Parent job", "Status Reason", "Resubmitted",
      "Job RSL", "Subjob Id Generation Seed", "Status Enter Time",
      "All States Enter Time",
      "Subjob Failed", "User Tags Names", "User Tags Values"
  };

  static final String[] jobStatusJobType = {
      "Job", "DAG"
  };

  static final String[] jobStateNames = {
      "undefined", "Submitted", "Waiting", "Ready", "Scheduled", "Running",
      "Done", "Cleared", "Aborted", "Cancelled", "Unknown", "Purged"
  };
  ...
  private void jbInit() throws Exception {
    ...
    jScrollPaneAttributes.setCorner(JScrollPane.LOWER_LEFT_CORNER, utilityPanel);

    jScrollPaneAttributes.setRowHeaderView(jTableHeader);
    jScrollPaneAttributes.setCorner(JScrollPane.UPPER_LEFT_CORNER,
    ...
    jPanelJob.setBorder(new TitledBorder(new EtchedBorder(), " Common Info ", 0,
        0, null, GraphicUtils.TITLED_ETCHED_BORDER_COLOR));

    jPanelJob.add(jLabelStatus, GraphicUtils.setGridBagConstraints(
        gbc, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelJob.add(jTextFieldStatus, GraphicUtils.setGridBagConstraints(
        gbc, 1, 0, 1, 1, 0.4, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelJob.add(jLabelStateEnterTime, GraphicUtils.setGridBagConstraints(
    ...
    jPanelJob.add(jLabelStatusReason, GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_END,
        GridBagConstraints.NONE, null, 0, 0));
    ...
    jPanelJob.add(jLabelJobType, GraphicUtils.setGridBagConstraints(
        gbc, 0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelJob.add(jTextFieldJobType, GraphicUtils.setGridBagConstraints(
        gbc, 1, 2, 2, 1, 0.6, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelJob.add(jLabelExitCode, GraphicUtils.setGridBagConstraints(
        gbc, 3, 2, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelJob.add(jTextFieldExitCode, GraphicUtils.setGridBagConstraints(
        gbc, 4, 2, 1, 1, 0.4, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelJob.add(jLabelOwner, GraphicUtils.setGridBagConstraints(
        gbc, 0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_END,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelJob.add(jScrollPaneJobOwner, GraphicUtils.setGridBagConstraints(
        gbc, 1, 3, 4, 1, 1.0, 0.5, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 0));

    jPanelJob.add(jLabelNetworkServer, GraphicUtils.setGridBagConstraints(
        gbc, 0, 4, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));
    ...
    jPanelJob.add(jLabelDestination, GraphicUtils.setGridBagConstraints(
        gbc, 0, 5, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));
    ...
    jPanelAttributes.setBorder(new TitledBorder(new EtchedBorder(),
        " Detailed Info ", 0, 0,
        null, GraphicUtils.TITLED_ETCHED_BORDER_COLOR));
    ...
    jPanelMain.add(jPanelLastUpdate, GraphicUtils.setGridBagConstraints(
        gbc, 0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, new Insets(1, 1, 1, 1), 0, 0));

    jPanelMain.add(jPanelJobId, GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 1, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelMain.add(jPanelJob, GraphicUtils.setGridBagConstraints(
        gbc, 0, 2, 1, 1, 1.0, 0.2, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 0));

    jPanelMain.add(jPanelAttributes, GraphicUtils.setGridBagConstraints(
        gbc, 0, 3, 1, 1, 1.0, 0.8, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 0));
    ...
    jPanelMain.add(jPanelButton2, GraphicUtils.setGridBagConstraints(
        gbc, 0, 5, 1, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));
    ...
    if ((text != null) && !text.equals("")) {
      ...
      try {
        ...
        JOptionPane.showOptionDialog(StatusDetailsFrame.this,
            Utils.UNESPECTED_ERROR + ex.getMessage(),
            Utils.ERROR_MSG_TXT,
  ...
  void setJobStatus(JobStatus jobStatus) {
    ...
    jTextFieldNetworkServer.setText(jobStatus.getValString(JobStatus.
        NETWORK_SERVER));
    ...
    jTextFieldDestination.setText(jobStatus.getValString(JobStatus.DESTINATION));
    jTextFieldOwner.setText(jobStatus.getValString(JobStatus.OWNER));
    jTextFieldStateEnterTime.setText(Utils.toTime(jobStatus.getValString(
        JobStatus.STATE_ENTER_TIME)));
    jTextFieldExitCode.setText(Integer.toString(jobStatus.getValInt(JobStatus.
        EXIT_CODE)));
    ...
    if ((value == null) || value.equals("") || !value.trim().substring(0,
        1).equals("[")) {
  ...
  void setTableStatusAttributes(JobStatus jobStatus) {
    ...
    jTableValueColumnWidth = (jTableValueColumnWidth > jTableValueWidth)
        ? jTableValueColumnWidth : jTableValueWidth;
