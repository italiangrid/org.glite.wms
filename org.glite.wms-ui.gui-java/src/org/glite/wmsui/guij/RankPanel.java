...
public class RankPanel extends JPanel {
  ...
  private void jbInit() throws Exception {
    ...
    jCheckBoxRankingPolicies.setForeground(GraphicUtils.TITLED_ETCHED_BORDER_COLOR);
    jCheckBoxRankingPolicies.addActionListener(new java.awt.event.
        ActionListener() {
      public void actionPerformed(ActionEvent e) {
    ...
    jRadioButtonShortestTimeToTraverseQueue.setText(
        "Shortest Time to Traverse Queue");
    ...
    jPanelRankingPolicies.add(jRadioButtonShortestTimeToTraverseQueue,
        ...
        gbc, 0, 0, 1, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jRadioButtonGreatestAvailableCPUTime,
        GraphicUtils.setGridBagConstraints(
        gbc, 1, 0, 2, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jRadioButtonGreatestMainMemory,
        GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 1, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jRadioButtonBestBenchmark,
        GraphicUtils.setGridBagConstraints(
        gbc, 1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jComboBoxBestBenchmark,
        GraphicUtils.setGridBagConstraints(
        gbc, 2, 1, 1, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jRadioButtonMaxNumberOfFreeCPUs,
        GraphicUtils.setGridBagConstraints(
        gbc, 0, 2, 1, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jRadioButtonMinNumberOfHandledJobs,
        GraphicUtils.setGridBagConstraints(
        gbc, 1, 2, 2, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelRankingPolicies.add(jRadioButtonDataAccessCost,
        GraphicUtils.setGridBagConstraints(
        gbc, 0, 3, 1, 1, 0.5, 0.0, GridBagConstraints.WEST,
        GridBagConstraints.NONE, null, 0, 0));
    ...
    this.add(rankAdvancedPanel, GraphicUtils.setGridBagConstraints(
        gbc, 0, 2, 1, 1, 0.0, 1.0, GridBagConstraints.FIRST_LINE_START,
  ...
  void jButtonAdvancedEvent(ActionEvent e) {
    if (jButtonAdvanced.getText().equals("Advanced >>")) {
      ...
      if (!result.equals("")
          && !result.equals(Jdl.RANK + " = " + GUIGlobalVars.getGUIConfVarRank()
          + ";")
          && !result.equals(Jdl.RANK + " = (" + GUIGlobalVars.getGUIConfVarRank()
          + ");")) {
      ...
      String result = rankAdvancedPanel.jButtonRankAdvancedPanelViewEvent(false,
          false, null).trim();
      if (!result.equals("") &&
  ...
  String jButtonRankViewEvent(boolean showWarningMsg, boolean showErrorMsg,
    ...
    if (jCheckBoxRankingPolicies.isSelected()) {
      if (jRadioButtonShortestTimeToTraverseQueue.isSelected()) {
        result = "- other." + exprAttributesName.getValue("timeToTraverseQueue");
        ...
        if (jComboBoxBestBenchmark.isVisible()) {
          String selectedBestBenchmark = jComboBoxBestBenchmark.getSelectedItem().
              toString();
    ...
    String checkResult = result.equals(fuzzyResult + Jdl.RANK + " = "
        ...
        ? (result + jint.jButtonDataReqViewEvent(false, false, null)) : result;
    ...
    if (!errorMsg.equals("") && showErrorMsg) {
      GraphicUtils.showOptionDialogMsg(RankPanel.this, errorMsg, Utils.ERROR_MSG_TXT,
  ...
  void enableAttributeFromConfFile() {
    String shortestTimeToTraverseQueue = exprAttributesName.getValue(
        "timeToTraverseQueue");
    ...
    String greatestAvailableCPUTime = exprAttributesName.getValue(
        "maxAvailableCPUTime");
    ...
    if ((bestBenchmarkInt == null) || (bestBenchmarkInt.trim().equals(""))) {
      ...
      if ((bestBenchmarkFloat != null) && !(bestBenchmarkFloat.trim().equals(""))) {
        jComboBoxBestBenchmark.setVisible(true);
      }
    }
    ...
    String minNumberOfHandledJobs = exprAttributesName.getValue(
        "minNumberOfHandledJobs");
...
class JPanelCheckBox extends JPanel {
  ...
  void jbInit() throws Exception {
    ...
    insidePanel.add(jPanelFill, GraphicUtils.setGridBagConstraints(
        gbc, 0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    insidePanel.add(this.innerPanel, GraphicUtils.setGridBagConstraints(
