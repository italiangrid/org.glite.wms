...
*/
  ...
  public void setUnknownText(String text) {
    ...
    try {
      ...
      if(isDebugging) jae.printStackTrace();
    } catch (Exception e) {
      if(isDebugging) e.printStackTrace();
      JOptionPane.showOptionDialog(UnknownPanel.this,
                                   e.getMessage(),
                                   Utils.ERROR_MSG_TXT,
  ...
  String jButtonUnknownViewEvent(boolean showWarningMsg, boolean showErrorMsg, ActionEvent ae) {
    this.errorMsg = "";
    this.warningMsg = "";
    Ad adCheck = new Ad();
    try {
      ...
      for (int i = 0; i < Utils.guiAttributesArray.length; i++) {
        if (adCheck.hasAttribute(Utils.guiAttributesArray[i])) {
          this.errorMsg += "- " + Utils.GUI_PANEL_NAMES[7] + " panel cannot define '"
              + Utils.guiAttributesArray[i] + "' attribute\n";
        }
      }
    ...
    if (!this.errorMsg.equals("") && showErrorMsg) {
      GraphicUtils.showOptionDialogMsg(UnknownPanel.this, errorMsg, Utils.ERROR_MSG_TXT,
