...
public class JobOutputDataPanel extends JPanel {
  ...
  private void jbInit() throws Exception {
    ...
    jTextFieldStorageElement.addFocusListener(new java.awt.event.FocusAdapter() {
      public void focusLost(FocusEvent e) {
        GraphicUtils.jTextFieldDeselect(jTextFieldStorageElement);
      }
    });
    jTextFieldLogicalFileName.setText("");
    jTextFieldLogicalFileName.addFocusListener(new java.awt.event.FocusAdapter() {
      public void focusLost(FocusEvent e) {
    ...
    jPanelOutputData.add(jTextFieldOutputFile, GraphicUtils.setGridBagConstraints(
        gbc, 1, 0, 2, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelOutputData.add(jLabelStorageElement, GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelOutputData.add(jTextFieldStorageElement, GraphicUtils.setGridBagConstraints(
        gbc, 1, 1, 2, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelOutputData.add(jLabelLogicalFileName, GraphicUtils.setGridBagConstraints(
        gbc, 0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
        GridBagConstraints.NONE, null, 0, 0));

    jPanelOutputData.add(jTextFieldLogicalFileName, GraphicUtils.setGridBagConstraints(
        gbc, 1, 2, 2, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jScrollPaneOutputData.getViewport().add(jTableOutputData, null);
    jScrollPaneOutputData.getViewport().setBackground(Color.white);
    jPanelOutputData.add(jScrollPaneOutputData, GraphicUtils.setGridBagConstraints(
        gbc, 0, 3, 2, 4, 1.0, 1.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 0));

    jPanelOutputData.add(jButtonAdd, GraphicUtils.setGridBagConstraints(
        gbc, 2, 3, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelOutputData.add(jButtonEdit, GraphicUtils.setGridBagConstraints(
        gbc, 2, 4, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelOutputData.add(jButtonRemove, GraphicUtils.setGridBagConstraints(
        gbc, 2, 5, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelOutputData.add(jButtonClear, GraphicUtils.setGridBagConstraints(
        gbc, 2, 6, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));
    ...
    this.setLayout(gbl);
    GraphicUtils.setDefaultGridBagConstraints(gbc);
    this.add(jPanelOutputData, GraphicUtils.setGridBagConstraints(
        gbc, 0, 0, 1, 1, 1.0, 0.3, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, new Insets(1, 1, 1, 1), 0, 0));

    this.add(jPanelDataROSE, GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 1, 1, 0.0, 0.7, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));
  }
    ...
    if (!outputFile.equals("")) {
      if (!jobTableModel.isElementPresentInColumn(outputFile,
        ...
        if (!jobTableModel.isRowPresent(outputFile + storageElement +
          ...
          JOptionPane.showOptionDialog(JobOutputDataPanel.this,
              "Inserted values are already present",
              Utils.ERROR_MSG_TXT,
              JOptionPane.DEFAULT_OPTION,
        ...
        JOptionPane.showOptionDialog(JobOutputDataPanel.this,
            "Inserted Output File is already present",
            Utils.WARNING_MSG_TXT,
            JOptionPane.DEFAULT_OPTION,
            JOptionPane.WARNING_MESSAGE,
            null, null, null);
      ...
      JOptionPane.showOptionDialog(JobOutputDataPanel.this,
          "Output File field cannot be blank",
          Utils.ERROR_MSG_TXT,
          JOptionPane.DEFAULT_OPTION,
          JOptionPane.ERROR_MESSAGE,
          null, null, null);
  ...
  String jButtonJobOutputDataViewEvent(boolean showWarningMsg,
    ...
    for (int i = 0; i < rowCount; i++) {
      outputFile = jobTableModel.getValueAt(i,
          OUTPUT_FILE_COLUMN_INDEX).toString().trim();
      storageElement = jobTableModel.getValueAt(i,
          STORAGE_ELEMENT_COLUMN_INDEX).toString().trim();
    ...
    if (!outputDataResult.equals("")) {
      outputDataResult = outputDataResult.substring(0,
          outputDataResult.lastIndexOf(","));
      if (rowCount == 1) {
    ...
    for (int i = 0; i < adVector.size(); i++) {
      ...
      try {
        logicalFileName = ad.getStringValue(Jdl.OD_LOGICAL_FILENAME).get(0).
            toString()
            .substring(JobInputDataPanel.LFN_PREFIX.length());
      } catch (NoSuchFieldException e) {
      // Attribute not present in jobad.
      } catch (Exception e) {
        if (isDebugging) {
