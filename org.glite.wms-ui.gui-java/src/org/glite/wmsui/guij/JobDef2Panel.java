...
public class JobDef2Panel extends JPanel {
  ...
  private void jbInit() throws Exception {
    ...
    jButtonRemoveEnvironment.addActionListener(new java.awt.event.
        ActionListener() {
    ...
    jButtonClearEnvironment.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
    ...
    jButtonReplaceEnvironment.addActionListener(new java.awt.event.
        ActionListener() {
      public void actionPerformed(ActionEvent e) {
    ...
    jTextFieldRetryCount.addFocusListener(new java.awt.event.FocusAdapter() {
      public void focusLost(FocusEvent e) {
        if (GUIGlobalVars.getGUIConfVarRetryCount() !=
            GUIGlobalVars.NO_RETRY_COUNT) {
          GraphicUtils.jTextFieldFocusLost(jTextFieldRetryCount, Utils.INTEGER,
              Integer.toString(GUIGlobalVars.
              getGUIConfVarRetryCount()),
              Utils.RETRYCOUNT_MIN_VAL,
    ...
    jPanelEnvironment.add(jTextFieldVarName, GraphicUtils.setGridBagConstraints(
        gbc, 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 160, 0));

    jPanelEnvironment.add(jLabelValue, GraphicUtils.setGridBagConstraints(
    ...
    jPanelEnvironment.add(jScrollPaneEnvironment, GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 4, 4, 1.0, 1.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 0));

    jPanelEnvironment.add(jButtonAddEnvironment, GraphicUtils.setGridBagConstraints(
        gbc, 4, 1, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelEnvironment.add(jButtonReplaceEnvironment,
        GraphicUtils.setGridBagConstraints(
        gbc, 4, 2, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelEnvironment.add(jButtonRemoveEnvironment, GraphicUtils.setGridBagConstraints(
        gbc, 4, 3, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    jPanelEnvironment.add(jButtonClearEnvironment, GraphicUtils.setGridBagConstraints(
        gbc, 4, 4, 1, 1, 0.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));
    ...
    jPanelMyProxyServer.setBorder(new TitledBorder(new EtchedBorder(),
        " Server for Credential Delegation ( MyProxyServer ) ", 0, 0,
        null, GraphicUtils.TITLED_ETCHED_BORDER_COLOR));

    jPanelMyProxyServer.add(jTextFieldMyProxyServer,
        GraphicUtils.setGridBagConstraints(
    ...
    jPanelHLRLocation.setBorder(new TitledBorder(new EtchedBorder(),
        " HLRLocation ", 0, 0, null,
        GraphicUtils.TITLED_ETCHED_BORDER_COLOR));

    jPanelHLRLocation.add(jTextFieldHLRLocation, GraphicUtils.setGridBagConstraints(
        gbc, 0, 0, 1, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));
    ...
    jTextFieldRetryCount.setPreferredSize( GraphicUtils.NUMERIC_TEXT_FIELD_DIMENSION);
    ...
    this.add(jPanelEnvironment, GraphicUtils.setGridBagConstraints(
        gbc, 0, 0, 2, 1, 1.0, 0.4, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 0));

    this.add(jPanelMyProxyServer, GraphicUtils.setGridBagConstraints(
        gbc, 0, 1, 2, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    this.add(jPanelHLRLocation, GraphicUtils.setGridBagConstraints(
        gbc, 0, 2, 2, 1, 1.0, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 0));

    this.add(jPanelRetryCount, GraphicUtils.setGridBagConstraints(
        gbc, 0, 3, 1, 1, 0.3, 0.6, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.HORIZONTAL, null, 0, 10));

    this.add(new JPanel(), GraphicUtils.setGridBagConstraints(
        gbc, 1, 3, 1, 1, 0.7, 0.0, GridBagConstraints.FIRST_LINE_START,
        GridBagConstraints.BOTH, null, 0, 10));
  ...
  String jButtonJobDef2ViewEvent(boolean showWarningMsg, boolean showErrorMsg,
    ...
    if (itemsCount != 0) {
      ...
      if (itemsCount == 1) {
        result += "\"" + jTableEnvironment.getValueAt(0,
            VARIABLE_NAME_COLUMN_INDEX).toString() + "="
            + jTableEnvironment.getValueAt(0,
            VARIABLE_VALUE_COLUMN_INDEX).toString() + "\";\n";
      } else {
        result += "{";
        for (int i = 0; i < itemsCount - 1; i++) {
          result += "\"" + jTableEnvironment.getValueAt(i,
              VARIABLE_NAME_COLUMN_INDEX).toString() + "="
              + jTableEnvironment.getValueAt(i,
              VARIABLE_VALUE_COLUMN_INDEX).toString() + "\", ";
        }
        result += "\"" + jTableEnvironment.getValueAt(itemsCount - 1,
            VARIABLE_NAME_COLUMN_INDEX).toString() + "="
    ...
    if (jCheckBoxRetryCount.isSelected()) {
      if (checkAttributeSet(Jdl.RETRYCOUNT,
          Integer.parseInt(jTextFieldRetryCount.getText().
          trim(), 10))) {
        result += Jdl.RETRYCOUNT + " = " + jTextFieldRetryCount.getText().trim()
            + ";\n";
      }
    }
  ...
  void jButtonAddEnvironmentEvent(ActionEvent e) {
    ...
    if ((!varName.equals("")) && (value.equals(""))) {
      ...
      if (!jobTableModel.isElementPresentInColumn(varName,
        ...
        jTextFieldVarName.grabFocus();
        JOptionPane.showOptionDialog(JobDef2Panel.this,
            "Variable '" + varName + "' is alredy defined",
            Utils.ERROR_MSG_TXT,
            JOptionPane.DEFAULT_OPTION,
            JOptionPane.ERROR_MESSAGE,
            null, null, null);
  ...
  void jButtonClearEnvironmentEvent(ActionEvent e) {
    ...
    if (jobTableModel.getRowCount() != 0) {
      int choice = JOptionPane.showOptionDialog(component,
          "Clear Environment Variables Table?",
          "Confirm Clear",
  ...
  void jButtonRemoveEnvironmentEvent(ActionEvent e) {
    ...
    if (selectedRowCount != 0) {
      ...
      JOptionPane.showOptionDialog(JobDef2Panel.this,
          "You have to select a table row first",
  ...
  void jButtonReplaceEvent(ActionEvent ae) {
    ...
    if (!varName.equals("")) {
      if (!selectedVarName.equals(varName)
          ...
          VARIABLE_NAME_COLUMN_INDEX)) {
        JOptionPane.showOptionDialog(JobDef2Panel.this,
            "Inserted Variable is already present",
            Utils.ERROR_MSG_TXT,
            JOptionPane.DEFAULT_OPTION,
            JOptionPane.ERROR_MESSAGE,
            null, null, null);
  ...
  void jButtonEditEvent(ActionEvent e) {
    ...
    if (selectedRowCount == 1) {
      ...
      JOptionPane.showOptionDialog(JobDef2Panel.this,
          "You have to select a single table row",
          Utils.INFORMATION_MSG_TXT,
          JOptionPane.DEFAULT_OPTION,
          JOptionPane.INFORMATION_MESSAGE,
          null, null, null);
