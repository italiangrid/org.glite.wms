...
char const XACML_COMMONAUTHZ_PROFILE_1_1[] = "http://glite.org/xacml/profile/grid-ce/1.0";
char const XACML_DCISEC_ATTRIBUTE_PROFILE_ID[] = "http://glite.org/xacml/attribute/profile-id";
std::string XACML_DCISEC_ATTRIBUTE_USER_ID("http://glite.org/xacml/attribute/user-id");
std::string XACML_DCISEC_ATTRIBUTE_GROUP_ID("http://glite.org/xacml/attribute/group-id");
std::string XACML_DCISEC_ATTRIBUTE_GROUP_ID_PRIMARY("http://glite.org/xacml/attribute/group-id/primary");
...
std::string read_certchain(std::string filename)
{
   ...
   while (infile.good()) {
      ...
      if (CERT_END == line) {
         cert_part = false; // skip the private key
      }
   }
