...
"""
 This method returns the current JSUI version
"""
def check_version():
   path=info.prefix
   if path:
      confFile=path+"/etc/glite_wmsui_cmd_var.conf"
      version= UIutils.parse_file([confFile,"Version"])
      if version[0]:
         return version[1]
   return ""
"""
     ...
     while line:
       ...
       try:
         ...
         if tag.strip()=="UI_VERSION":
            UIutils.info.version = ver
            break
       except:
         pass
       line=f.readline()
     f.close()
     if UIutils.info.version =="":
...
def checkConfVo(conf , virtualOrg, *jobad):
	...
	# --vo option specified
	if virtualOrg:
		confDir = UIutils.info.prefix + sep + "etc" + sep +virtualOrg.lower()
	...
	#   VirtualOrganisation attribute specified in JDL file
		...
		confDir = UIutils.info.prefix + sep + "etc" + sep +voName.lower()
		final = confDir +  sep + voDefaultName
		vomsrc="JDL"
	...
	if defaultVo:
		...
		if not final:
			voName=defaultVo
			confDir= UIutils.info.prefix + sep + "etc" + sep +defaultVo.lower()
			final=confDir +sep + voDefaultName
	...
	if final :
		...
		if  (vomsrc ) and (defaultVo )and (voName) and ( voName.lower()!=defaultVo.lower() ):
			...
			if UIutils.info.noint:
				...
				answ = UIutils.questionYN(question)
...
def checkConf( conf , logPath ):
  ...
  if logPath:
     ...
     if os.path.isfile ( logFile):
        ...
        logFile = UIutils.create_err_log ( info.prgname )
...
def check_outFile(out_file,*submit):
  ...
  if not os.path.isdir(out_file[:sind]): #the path doesn't exist
          UIutils.errMsg("Error","UI_DIR_NOT_FOUND",out_file[:sind])
          UIutils.exit(1)
  ...
  if os.path.isfile(out_file): #the file already exists
    ...
    if submit:
       ...
       if line == "###Submitted Job Ids###":
           return [0,out_file,0]
    UIutils.errMsg("Warning","UI_FILE_EXISTS",out_file)
    if info.noint: #No-int active
       UIutils.errMsg("Warning","UI_SKIP_QUESTION","overwriting the File "+out_file)
...
def checkVomsExtension  (proxyfile, printERRORS):
	...
	if UIutils.info.debug and printERRORS:
...
def check_noint(argv):
  ...
  if  ("--debug" in argv) or("-debug" in argv):
    set=1
    info.debug=1
    info.logFile=UIutils.create_err_log(info.prgname)
...
def unknown( ad  ):
	unknown=""
	# stick to preserve lower case:
	known = ["requirements","fuzzyrank","nodenumber","listenerport","rank", \
	"jobsteps","currentstep","stepweigth","partitionnumber","prejob","postjob" \
	"submitto","executable","stdoutput","stdinput","stderror","arguments","myproxyserver", \
	"jobtype","outputse", "type","virtualorganisation","hlrlocation","outputsandbox", \
	"inputsandbox", "environment", "inputdata", "outputdata","dataaccessprotocol", \
	"retrycount","edg_jobid","lrms_type", "storageindex", "usertags", "expirytime", \
	"datacatalog", "stepweight", "nodescollocation", "movetoce", "jobprovenance", \
	"shallowretrycount","prolog","prologarguments","cpunumber", "nodes",
	"defaultnoderetrycount","defaultnodeshallowretrycount","dependencies","max_running_nodes", \
	"mwversion","significantattributes"]


	for att in ad.attributes():
		if not att.lower() in known:
			unknown+="- " + att +"\n"
	if unknown:
		errMsg ( "Warning" , "UI_JDL_UNKNOWN" , unknown )
