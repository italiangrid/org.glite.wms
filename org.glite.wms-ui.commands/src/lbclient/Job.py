...
def getEvents(jobid, level):
	...
	for eventNumber in range(eventsNumber):
		...
		if err:
			# Print the error and terminate
			UIutils.errMsg("**** Error: API_NATIVE_ERROR ****", "Error while calling the \"Eve::getEventAttributes\" native api", apiMsg)
			exit(1)
...
def queryStates(jobids, includes, excludes, userTags, issuer, fromT, toT, level):
	...
	if jobids:
		...
		lbAddresses = UIutils.getLBs()

		if not lbAddresses:
			# TODO: Pass these two parameters from outside
			UIutils.queryLBs(voName, lbAddresses)
...
class JobState:
  ...
  def check (self):
	...
	if errors:
		UIutils.errMsg("Error","UI_JDL_ERROR", errors)
		exit(1)
  """
  Load a chkpt file
  """
  def fromFile( self ,stateFile ):
	if not os.path.isfile(   stateFile ):
			UIutils.errMsg("Error","UI_FILE_NOT_FOUND",submissionFile)
			exit(1)
  ...
  def fromString(self, stateStr):
	...
	if err:
		UIutils.errMsg("Error","UI_JDL_ERROR", "Unable to convert into a jobstate instance.\n"+ apiMsg )
		exit(1)
  """
	...
	if  jobad.hasKey (self.JOB_STEPS ):
			...
			if val:
				...
				if val:
					...
					UIutils.errMsg("Error","UI_JDL_ERROR", "Wrong type caught for " + self.JOB_STEPS  )
...
def getJobIdfromFile (inFile ):
     ...
     if not os.path.isfile(inFile):
       ...
       for job in f.readlines():
           ...
           if ( job.find ('#*') == 0):
              ...
              if jobid.fromString(job):
                 UIutils.errMsg( 'Warning','UI_WRONG_JOBID_FORMAT',job)
                 sys_exit = -1
              else:
                 if job in str_jobs:
                    UIutils.errMsg( 'Warning','UI_JOBID_REPEAT',job)
                    sys_exit = -1
                 else:
       ...
       f.close
     if not parsed:
          UIutils.errMsg('Error','UI_NO_JOBID')
          exit(1)
...
class Shadow:
  ...
  def  checkRootPath(self):
	...
	rootDir  = UIutils.FILESEP + "tmp"
	rd = UIutils.solvePath( UIutils.info.confAd.getStringValue ("ListenerStorage")[0] )
	if rd:
		rootDir = rd
	if  not os.path.isdir( rootDir ):
		errMsg('Error','UI_DIR_NOT_FOUND', rootDir)
		exit(1)
	self.pipeRoot =  rootDir +UIutils.FILESEP + "listener-" + self.jobid.unique
	foundPrevious =0
  ...
  def console( self, port=0):
	shPath = UIutils.info.prefix + UIutils.FILESEP +"bin/"
	...
	for i in range (retryPort):
		...
		if errConsoleInfo == ALREADY_USED_PORT:
			if fromPort and toPort and UIutils.info.debug:
			...
			if fromPort and toPort and UIutils.info.debug:
				UIutils.print_message(UIutils.info.logFile,"Firewall range port found. listening port forced to: " +repr (port )  )
			break
...
class Event:
	...
	def getAttribute(self, eventAttribute):
		...
		if eventAttribute < 0 or eventAttribute >= EVENT_ATTR_MAX:
			# Print the error and terminate
			UIutils.errMsg("Error:", "Event number " + str(eventAttribute) + " out of range 0:" + str(EVENT_ATTR_MAX-1))
			exit(1)
...
class JobStatus:
	...
	def getAttribute(self, statusAttribute):
		...
		if statusAttribute < 0 or statusAttribute >= STATE_ATTR_MAX:
			# Print the error and terminate
			UIutils.errMsg("Error:", "Status number " + str(statusAttribute) + " out of range 0:" + str(STATE_ATTR_MAX-1))
			exit(1)
