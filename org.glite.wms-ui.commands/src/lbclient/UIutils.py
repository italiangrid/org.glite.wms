...
def getRandom():
	return int(math.ceil(random.random()*int(time.time())))
...
def insertFQAN(ad):
	"""
	Determine the user proxy and extract the FQAN value
	"""
	proxy_file_name=getProxyFileName()
	if os.path.isfile(proxy_file_name):
		#Try to look up the default Vo in a previous existing proxy
		uc = UCredential(proxy_file_name)
		defaultFQAN= uc.getDefaultFQAN()
		if defaultFQAN:
			ad.setAttributeStr("VOMS_FQAN",defaultFQAN)
		else:
			errMsg('Warning','UI_NO_VOMS',"Unable to retrieve FQAN from proxy certificate: " + proxy_file_name)
...
"""
This method parses an (existent) file  and looks for
several input tags
"""
def parse_file(tags):
   ...
   if line:
    if not line[0]=='#':
      ...
      #Follows from previous line
        result[keep],keep=check_end(result[keep] + line,keep)
      else:
        args=line.split('=',1)
        if len(args)==2:
           code,value =args
           ...
           for i in range(1,len(result)):
             if code == tags[i]:
               found=found+1
               result[i],keep=check_end(value,i)
               break
   line=f.readline()
  result[0]=found
  f.close()
...
"""
This method is used by parse_file
"""
def check_end(word,ind):
  lon = len(word)-1
  if word[lon]=='\\': #The line continues
    word=word[0:lon].strip() +' '
    return [word,ind]
  else:               #The line doesn't continue
    return [word,0]
def print_help(msg):
   if not info.logFile:
...
Finds a value among the specified options
 usage mode: userTags = UIutils.findVal(["--user-tag"], options, values)
...
def checkNs( ns ):
	...
	port_ind = ns.find (":")
	if port_ind <0:
		port = DEFAULT_NS_PORT
		port_ind= len ( ns )
	else:
		port =   ns  [  port_ind+1:  ]
	#Check the port:
	try:
		ns=resolveHostName(ns[prot_ind : port_ind])
		port = int (  port )
	except:
		errMsg('Warning','UI_CONFIG_ATTRIBUTE', "NSAddresses, host not found: "+ns)
		return [1, ""]
	address = prot+ns
	return [0   , [address , port]   ]
...
def querySd(voName, serviceType):
	"""
	Used both by queryNS and by queryLB
	"""
...
def queryNSs (voName, alreadyTested):
	"""
	Retreve NS list from service Discovery
	return the list of found NS addresses
	"""
	NS_SERVICE_TYPE="org.glite.wms.NetworkServer"
	nss=[]
	# prompt user
	if info.noint:
		# Authomatically query NS
		errMsg("Warning","UI_SKIP_QUESTION","Authomatically query the Service Discovery for more Network Addresses")
	elif questionYN("Do you wish to query the Service Discovery for more Network Addresses?"):
		...
		return nss
	# Query SD replied
	nssToCheck = querySd(voName,NS_SERVICE_TYPE)
	for ns in nssToCheck:
		if not ns in alreadyTested:
			# check&append only if not yet tested
			err , nsAdd = checkNs (ns)
			if not err:
				nss.append (nsAdd)
	return nss
...
"""
Retreve NS list from configuration file
"""
def getNSs ():
	# check values
	nss = []
	nssToCheck = info.confAdVo.getStringValue ("NSAddresses")
	if not  nssToCheck:
		errMsg('Error','UI_CONFIG_ATTRIBUTE', "NSAddresses","Unable to find any")
		return nss
	# cycle over found NS addresses:
	for ns in nssToCheck:
		err , nsAdd = checkNs (ns)
		if not err:
			nss.append (nsAdd)
...
def cycleLbs(status, lbAddresses, sys_exit):
	"""
	Query specified LB addresses for submitted jobs
	INPUT:
		status: a Status instance
		lbdAddresses: the list of addresses to be queried
		sys_exit: the currend sys_exit value
	OUTPUT:
		sys_exit: input sys_exit value that may  have changed
	"""
	for addr in lbAddresses:
		lbHost , lbPort = addr
		wait_msg="\nNow Retrieving user's Job from LB server "+ \
		lbHost+":"+ repr(lbPort)+"\nPlease wait: this operation could take some seconds.\n"
		print wait_msg
		dbgMsg ("Status::getStates" , lbHost, lbPort)
		status.getStates(lbHost, lbPort)
		err , apiMsg = status.get_error ()
		if err:
			errMsg ( "Warning" , "API_NATIVE_ERROR" , "Status:getStates"  , apiMsg )
			sys_exit= -1
	return sys_exit

def cycleNS(ns, nsLogLevel, nsAddresses):
	"""
	Contact the specified ns Instances and
	initialise ns passed instance
	this method is used both by submit and by list-match
	INPUT
		ns NS() instance
		nsLogLevel number in range [0,7]
		nsAddresses list of addresses (strings) to be contacted
	OUTPUT
		nsFound 1/0 if address found/not found
		nsNum index of address inside list
		nsHost host of NS address
		multi attributes multi-valued requested list
	"""
	multi = []    # multi attributes found
	nsVisited=[]
	# Copy the provided addresses
	nsAddressesCopy=[]
	for nsStr in nsAddresses:
		nsAddressesCopy.append(nsStr)

	nsFound = 0   # flag indicating NS Successfully found
	nsHost=""
	nsPort=0
	nsNumBer   = -1  # Default Value for not-ordered ns

	while (len(nsAddressesCopy)):
		# pop up a random nsAddress bug #14354 fix
		nsAddress= nsAddressesCopy.pop(getRandom()%len(nsAddressesCopy))
		nsNumBer = nsAddresses.index(nsAddress)
		if nsAddress in nsVisited:
			# already queried NS, trying next one
			pass
		else:
			# Proceed with NS initialisation
			nsVisited.append(nsAddress)
			nsHost , nsPort = nsAddress
			dbgMsg ("NSClient::init" , nsHost , nsPort, nsLogLevel )
			ns.ns_init( nsHost , nsPort , nsLogLevel )
			err , apiMsg = ns.get_error ()
			if err:
				ns.ns_free()
				pass
			else:
				#Get multi attribute
				dbgMsg ("NSClient::multi")
				multi = ns.ns_multi()
				err , apiMsg = ns.get_error ()
				if err:
					errMsg ("Warning","API_NATIVE_ERROR","NSClient::multi", apiMsg)
					ns.ns_free()
				else:
					print_message( info.logFile, "Connecting to host " + nsHost + ", port " + repr(nsPort))
					nsFound = 1
					break
	return [nsFound, nsNumBer, nsHost, nsPort, multi]



"""
Set the default Logging timeout variables from configuration file
"""
def setLoggingTimeout():
   #Names of the environment variables to be set
   envs =["EDG_WL_LOG_TIMEOUT","EDG_WL_LOG_SYNC_TIMEOUT"]
   #Names of the variables as in the UI conf file
   vars= ["LoggingTimeout","LoggingSyncTimeout"]
   for i in range( len(vars)):
      var = ""
      try:
         var = os.environ[envs[i]]
      except:
         var = ""
      if not var:
         valueToSet = info.confAd.getIntValue(vars[i])
         if valueToSet:
              valueToSet=valueToSet[0]
              os.environ[envs[i]] = repr(valueToSet)
"""
...
"""
Prepare the jobad for submission by inserting default attributes
"""
def setDefaultAd( jobad, jobType ):
	if info.TYPE== 0:
		#JOB
		DEF_RANK = "defaultRank"
		DEF_REQ = "defaultReq"
	...
	name ="rank"
	val =""
	#MPI-CH pre-check: Rank MUST be equal to ZERO
	if not jobad.hasKey( name):
		if "mpich" in jobType:
			#Initialise with 0 value
			val = "other.GlueCEStateFreeCPUs"
		else:
			# Get the default value from:
			# 1) conf VO file
			# 2) normal conf file
			if info.confAdVo.hasKey( name ):
				val = info.confAdVo.getValueExpr ( name )
			elif info.confAd.hasKey( name ):
				val = info.confAd.getValueExpr ( name )
		if val:
			# Set the value into the JDL
			if info.TYPE:
				jobad.setAttributeStr  ( DEF_RANK  , val )
			else:
				jobad.setAttributeExpr( DEF_RANK , val )
			err, msg = jobad.get_error()
			if err:
				errMsg('Error','UI_CONFIG_ATTRIBUTE', name , msg)
				exit(1)
		else:
			errMsg('Error','UI_CONFIG_ATTRIBUTE', name , "Unable to find value")
			exit(1)
	"""
	Requirements
	"""
	name = "requirements"
	val = ""
	# Setting configuration default value (always needed)
	if info.confAdVo.hasKey( name):
		val =  info.confAdVo.getValueExpr ( name )
	elif info.confAd.hasKey( name):
		val =  info.confAd.getValueExpr ( name )
	if val:
		# Set the default Requiremtens attrbute
		if info.TYPE:
			jobad.setAttributeStr  (  DEF_REQ  , val )
		else:
			jobad.setAttributeExpr( DEF_REQ , val )
		err, msg = jobad.get_error()
		if err:
			errMsg('Error','UI_CONFIG_ATTRIBUTE', name , msg)
			exit(1)
	...
	VirtualOrganisation
	"""
	# Virtual Organisation: Get the default value from conf file
	name = "VirtualOrganisation"
	if not jobad.hasKey( name ):
		val = info.confAdVo.getStringValue ( name )
		if val:
			jobad.setAttributeStr( VIRTUAL_ORGANISATION , val[0] )
		else:
			errMsg('Error','UI_CONFIG_ATTRIBUTE', name,"Unable to find value")
			exit(1)
	"""
	Attribute(s): Default  HLR Location, MyProxyServer and RetryCount
	Conf File: UI VO conf file
	"""
	if info.TYPE==0:
		# Hlr Location: Get the default value from conf file
		for name in [ "HlrLocation" , "MyProxyServer"]:
			if not jobad.hasKey( name ):
				val = info.confAdVo.getStringValue ( name )
				if val and val[0]:
					# Set the value into the JDL
					jobad.setAttributeStr( name , val[0] )
		name = "RetryCount"
		if not jobad.hasKey( name ):
			val =  info.confAd.getIntValue ( name )
			# Set the value into the JDL
			if val:
				jobad.setAttributeInt( name ,  val[0] )
