...
namespace { // begin local namespace
  ...
  static int fetch_jobs_callback(void *param, int argc, char **argv, char **azColName){
    ...
    if( argv && argv[0] ) {
      ...
      for(int i = 0; i<=25; ++i) {// a database record for a CreamJob has 26 fields, as you can see in Transaction.cpp, but we excluded complete_cream_jobid from the query
	if( argv[i] )
...
void GetTerminatedJobs::execute( sqlite3* db ) throw ( DbOperationException& )
{
  ostringstream sqlcmd( "" );
  sqlcmd << "SELECT " <<CreamJob::get_query_fields() << " FROM jobs WHERE status=\'"
	   << api::job_statuses::CANCELLED 
	   << "\' OR status=\'"
	   << api::job_statuses::DONE_OK 
	   << "\' OR status=\'"
	   << api::job_statuses::DONE_FAILED
	   << "\' OR status=\'"
