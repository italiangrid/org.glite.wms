...
static int fetch_job_callback(void *param, int argc, char **argv, char **azColName){
  ...
  if ( argv && argv[0] ) 
    {
      for(int i = 0; i<=25; i++) {// a database record for a CreamJob has 27 fields, as you can see in Transaction.cpp, but we excluded the complete_cream_jobid from the query
	if( argv[i] )
