...
ice_util::iceCommandProxyRenewal::iceCommandProxyRenewal( ) :
...
//______________________________________________________________________________
void ice_util::iceCommandProxyRenewal::checkProxies()
{
  boost::recursive_mutex::scoped_lock M( ice_util::jobCache::mutex );

  for(ice_util::jobCache::iterator jobIt = m_cache->begin(); jobIt != m_cache->end(); ++jobIt) {
      if ( ! jobIt->is_active() ) 
    ...
    if( ::stat( jobIt->getUserProxyCertificate().c_str(), &buf) == 1 )
    {
      CREAM_SAFE_LOG(m_log_dev->errorStream() 
		     << "iceCommandProxyRenewal::checkProxies() - Cannot stat proxy file ["
	...
	CREAM_SAFE_LOG(m_log_dev->infoStream() 
		       << "iceCommandProxyRenewal::checkProxies() - Need to renew proxy  ["
		       << jobIt->getUserProxyCertificate() << "] for job ["
		       << jobIt->getCreamJobID() << "]"
		       << log4cpp::CategoryStream::ENDLINE);
	  ...
	  CREAM_SAFE_LOG( m_log_dev->errorStream() 
			  << "iceCommandProxyRenewal::checkProxies() - Proxy renew failed: ["
			  << ex.what() << "]"
			  << log4cpp::CategoryStream::ENDLINE);
        }
	
	CREAM_SAFE_LOG(m_log_dev->infoStream() 
		       << "iceCommandProxyRenewal::checkProxies() - Proxy renew SUCCESSFULL for job "
