...
void iceCommandJobKill::execute( ) throw()
{
  try {
    ...
    if( proxyTimeLeft < m_threshold_time && proxyTimeLeft > 5 ) {
	  ...
	  try {
	    this->killJob( proxyTimeLeft );
	  } catch(exception& ex) {
	    CREAM_SAFE_LOG (
                      m_log_dev->errorStream() 
                      << "iceCommandJobKill::execute() - killJob raised an exception: "
                      << ex.what()
                      << log4cpp::CategoryStream::ENDLINE);
	  } catch(...) {
	    CREAM_SAFE_LOG (
                      m_log_dev->errorStream() 
                      << "iceCommandJobKill::execute() - killJob raised an unknown exception."
                      << log4cpp::CategoryStream::ENDLINE);
	  }
    }
  } catch(exception& ex) {
...
//______________________________________________________________________________
void iceCommandJobKill::killJob( const time_t residual_proxy_time ) throw()
{
  try {
    ...
    CREAM_SAFE_LOG (
                      m_log_dev->errorStream() 
                      << "iceCommandJobKill::killJob() - Error"
                      << " killing job: Unknown exception catched."
                      << log4cpp::CategoryStream::ENDLINE);
  }
