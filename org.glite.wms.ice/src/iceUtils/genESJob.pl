...
#
# This script takes the template ESActivity.txt and generates the source header
# file ESActivity.h containing the ESActivity class with proper ctors, and setter/getter
# method. This is a convenient way to do, because the members of ESActivities are many and
...
@_data_methods = `cat ESActivity.txt`;
...
@_data_members = `cat ESActivity.txt`;
...
for($i = 0; $i < scalar @data_members; $i++) {
  ...
  $member = $pieces[2];
  $membertype = $pieces[1];
  print "\t\t    out += \"m_$member = \" + boost::lexical_cast<string>(m_$member) + \"\\n\";\n";
}
print "\t\t    return out;\n";
print "\t\t  };";
print "\n\n\t\t}; // class ESActivity";
