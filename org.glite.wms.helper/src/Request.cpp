...
public:
  explicit Impl(const classad::ClassAd* ad);
  ~Impl();
...
Request::Impl::Impl(const classad::ClassAd* ad)
  : m_state_machine(0),
    m_original_ad(ad),
...
try {
  ...
  std::auto_ptr<classad::ClassAd> resolved_ad(m_state_machine.next_step(m_current_ad));
