...
std::string get_proxy_distinguished_name(std::string const& proxy_file);
std::string get_tag(matchmaking::match_info const& info);
STACK_OF(X509) *load_chain(const char *certfile);
bool VOMS_proxy_init(const std::string& user_cert_file_name, Attributes& USER_attribs);
bool filter_gpbox_authorizations(matchmaking::match_table_t& suitable_CEs,
