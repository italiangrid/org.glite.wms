...
import socket
import wmsui-utils
from wmsui-utils import exit
from wmsui-utils import errMsg
from wmsui-utils import dbgMsg
...
def getEvents(jobid, level):
	...
	for eventNumber in range(eventsNumber):
		...
		if err:
			# Print the error and terminate
			wmsui-utils.errMsg("**** Error: API_NATIVE_ERROR ****", "Error while calling the \"Eve::getEventAttributes\" native api", apiMsg)
			exit(1)
...
def queryStates(jobids, includes, excludes, userTags, issuer, fromT, toT, level):
	...
	if jobids:
		...
		lbAddresses = wmsui-utils.getLBs()

		if not lbAddresses:
			# TODO: Pass these two parameters from outside
			wmsui-utils.queryLBs(voName, lbAddresses)
...
class JobState:
  ...
  def check (self):
	...
	if errors:
		wmsui-utils.errMsg("Error","UI_JDL_ERROR", errors)
		exit(1)
  """
  Load a chkpt file
  """
  def fromFile( self ,stateFile ):
	if not os.path.isfile(   stateFile ):
			wmsui-utils.errMsg("Error","UI_FILE_NOT_FOUND",submissionFile)
			exit(1)
  ...
  def fromString(self, stateStr):
	...
	if err:
		wmsui-utils.errMsg("Error","UI_JDL_ERROR", "Unable to convert into a jobstate instance.\n"+ apiMsg )
		exit(1)
  """
	...
	if  jobad.hasKey (self.JOB_STEPS ):
			...
			if val:
				...
				if val:
					...
					wmsui-utils.errMsg("Error","UI_JDL_ERROR", "Wrong type caught for " + self.JOB_STEPS  )
...
def getJobIdfromFile (inFile ):
     ...
     if not os.path.isfile(inFile):
       ...
       for job in f.readlines():
           ...
           if ( job.find ('#*') == 0):
              ...
              if jobid.fromString(job):
                 wmsui-utils.errMsg( 'Warning','UI_WRONG_JOBID_FORMAT',job)
                 sys_exit = -1
              else:
                 if job in str_jobs:
                    wmsui-utils.errMsg( 'Warning','UI_JOBID_REPEAT',job)
                    sys_exit = -1
                 else:
       ...
       f.close
     if not parsed:
          wmsui-utils.errMsg('Error','UI_NO_JOBID')
          exit(1)
...
class Shadow:
  ...
  def  checkRootPath(self):
	...
	rootDir  = wmsui-utils.FILESEP + "tmp"
	rd = wmsui-utils.solvePath(wmsui-utils.info.confAd.getStringValue ("ListenerStorage")[0] )
	if rd:
		rootDir = rd
	if  not os.path.isdir( rootDir ):
		errMsg('Error','UI_DIR_NOT_FOUND', rootDir)
		exit(1)
	self.pipeRoot =  rootDir +wmsui-utils.FILESEP + "listener-" + self.jobid.unique
	foundPrevious =0
  ...
  def console( self, port=0):
	shPath = wmsui-utils.info.prefix + wmsui-utils.FILESEP +"bin/"
	...
	for i in range (retryPort):
		...
		if errConsoleInfo == ALREADY_USED_PORT:
			if fromPort and toPort and wmsui-utils.info.debug:
			...
			if fromPort and toPort and wmsui-utils.info.debug:
				wmsui-utils.print_message(wmsui-utils.info.logFile,"Firewall range port found. listening port forced to: " +repr (port )  )
			break
...
class Event:
	...
	def getAttribute(self, eventAttribute):
		...
		if eventAttribute < 0 or eventAttribute >= EVENT_ATTR_MAX:
			# Print the error and terminate
			wmsui-utils.errMsg("Error:", "Event number " + str(eventAttribute) + " out of range 0:" + str(EVENT_ATTR_MAX-1))
			exit(1)
...
class JobStatus:
	...
	def getAttribute(self, statusAttribute):
		...
		if statusAttribute < 0 or statusAttribute >= STATE_ATTR_MAX:
			# Print the error and terminate
			wmsui-utils.errMsg("Error:", "Status number " + str(statusAttribute) + " out of range 0:" + str(STATE_ATTR_MAX-1))
			exit(1)
