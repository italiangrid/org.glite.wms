...
)
{
  ...
  try {
   ...
   if (match_result.size()!=0) {
     ...
     for (int i = 0; i < std::min(number_of_results, match_result.size()); ++i) {
  ...
  for (retry_count = 7; retry_count > 0; backoff_time*=2,retry_count--)
   {
    int write_retcod = write(fd,result_string.c_str()+written_size,write_size);
    if (write_retcod<0)
     {
      if (errno == EAGAIN) sleep(backoff_time);
      else break;
     }
