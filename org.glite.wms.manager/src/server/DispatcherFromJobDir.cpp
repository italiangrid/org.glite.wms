...
requests_type read_new_requests(utilities::JobDir& jd)
{
  requests_type result;

  utilities::JobDir::iterator b, e;
  boost::tie(b, e) = jd.new_entries();
  ...
  for ( ; b != e; ++b) {
    ...
    fs::path const old_file = jd.set_old(new_file);
...
{
  requests_type new_requests(read_new_requests(jd));
  ...
  for ( ; it != end; ++it) {
    ...
    boost::tie(command_, cleanup) = *it;

    classad::ClassAd& command_ad = *command_;
    ...
    try {
      ...
      if (it == tq.end()) {
        ...
        if (command == "jobsubmit" || command == "jobresubmit") {
          ...
          cleanup_guard.dismiss();
          RequestPtr request(new Request(command_ad, command, id, cleanup));
          Info("considering plan of " << request->id());
          write_end.write(request);
          tq.insert(std::make_pair(id.toString(), request));
...
struct DispatcherFromJobDir::Impl
{
...
DispatcherFromJobDir::DispatcherFromJobDir(
  std::string const& input,
  TaskQueue& tq
)
...
void DispatcherFromJobDir::operator()()
try {

  Info("Dispatcher: starting");

  std::string const& input(m_impl->m_input);
  TaskQueue& tq(m_impl->m_tq);

  fs::path const input_(input, fs::native);
  utilities::JobDir jd(input_);

  Info("Dispatcher: reading from " << input);
  ...
  while (!received_quit_signal()) {
    ...
    remove_done(tq);
    get_new_requests(jd, input, tq, write_end());
