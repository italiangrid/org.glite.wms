	...
	const string &proxy)
{
	...
	authorizer::WMPAuthorizer auth("putProxy", getDelegatedProxyPath(delegation_id));
	auth.authorize();
	...
	const string &delegation_id)
{
	...
	authorizer::WMPAuthorizer auth("renewProxyReq", getDelegatedProxyPath(delegation_id));
	auth.authorize();
